...
# تخصيص واجهة عصرية متوافقة مع الجوال ودعم RTL
st.markdown("""
<style>
...
with head_col1:
    with st.popover("⚙️"):
        st.markdown("#### ⚙️ تعديل الحسابات الافتراضية")
        snap_handle = st.text_input("👻 سناب شات:", value="Fig.fig.", key="snap")
        tiktok_handle = st.text_input("🎵 تيك توك:", value="arb.virtualventurex", key="tiktok")
        ig_handle = st.text_input("📸 إنستقرام:", value="arb.virtualventurex", key="ig")
        x_handle = st.text_input("𝕏 منصة إكس:", value="ahmadtawfir", key="x")
        yt_handle = st.text_input("▶️ يوتيوب:", value="abu10shaher", key="yt")

# تجميع وتجهيز الحسابات للعرض
saved_accounts = {}
...
if uploaded_file is not None:
    ...
    enable_stamp = st.checkbox("✨ وضع أحد حساباتي لتغطية الشعار القديم", value=True)
    stamp_filter = ""
